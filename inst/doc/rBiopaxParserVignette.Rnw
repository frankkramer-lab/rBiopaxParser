...
32, 37073 Göttingen, Germany. eMail: mail@frankkramer.de}}
\date{\today}
\maketitle
...
This package currently only supports Biopax Level 2, support for Biopax Level 3
is on the way!
%\newline
...
Currently only the NCI website is linked, with exports of the Pathway Interaction
Database (PID), BioCarta and Reactome available.
...
function reads in a BioPAX .owl file and generates the internal data.frame format
used in this package. This function can take a while with large BioPAX files like
...
of the ontology is BioPAX Level 3. This package currently only supports BioPAX
Level 2. Support for BioPAX Level 3 is planned in a future release.
...
and then converted to a data.frame. This allows for efficient indexing and selecting of
subsets of this data.frame.
...
like data.frame. The trick here is to move these instances into the main XML tree and
reference the specific instance with an rdf:resource attribute.

An excerpt of the internal data.frame of a biopax model, as created in the last section
of this document "Modifying BioPAX":
...
This data.frame represents instances as a collection of their properties. The
...
For comprehensive databases this data.frame can reach quite extensive sizes. The
data.frame itself can be accessed directly via the slot "df" of
...
a subset of the internal data.frame meeting these conditions. This functions is
...
The next type of functions return (compared to the internal data.frame)
...
biopax\$df, there are quite a few convenience functions to make life easier.
